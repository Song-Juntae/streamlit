...
with col6_2:
    ...
    if len(키워드) > 1:
        df_파이차트 = pd.DataFrame(마스크된데이터프레임['sentiment']마스크된데이터프레임['time'][마스크된데이터프레임['review_sentence'].str.contains('|'.join(키워드))].value_counts())
    if len(키워드) == 1:
        df_파이차트 = pd.DataFrame(마스크된데이터프레임['sentiment']마스크된데이터프레임['time'][마스크된데이터프레임['review_sentence'].str.contains(키워드[0])].value_counts())
